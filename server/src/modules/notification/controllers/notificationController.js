...
BEGIN
    ...
    CREATE UNIQUE INDEX UX_PUSHNOTIFICATION_TOKEN ON PUSHNOTIFICATION (DEVICE_TOKEN);
END`;

const INSERT_TOKEN_QUERY = `
MERGE PUSHNOTIFICATION AS target
...
SELECT DEVICE_TOKEN, DEVICE_TYPE FROM PUSHNOTIFICATION WHERE IS_ACTIVE = 1`;
...
exports.initializeTable = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: 'PUSHNOTIFICATION table initialized.' });
  } catch (error) {
