...
export function TransactionDetailList({ details, loading = false, onQuantityReceivedChange, onBagsReceivedChange }: TransactionDetailListProps) {
  const scheme = useColorScheme();
  const colors = Colors[scheme ?? 'light'];
  const [receivedQuantities, setReceivedQuantities] = React.useState<Record<number, string>>({});
  const [receivedBags, setReceivedBags] = React.useState<Record<number, string>>({});
        ...
        >
          ...
          <View style={[styles.cardHeader, { borderBottomColor: colors.divider }]}>
            ...
            <View style={styles.cardHeaderRight}>
              <View
                style={[styles.sameQuantityBtn, { backgroundColor: colors.primary + '15', borderColor: colors.primary }]}
              >
                <MaterialCommunityIcons name="content-copy" size={12} color={colors.primary} />
                <Text
                  style={[styles.sameQuantityBtnText, { color: colors.primary }]}
                  onPress={() => {
                    const issuedQty = detail['QUANTITY ISS.'] || 0;
                    const issuedBags = detail['BAGS ISS.'] || 0;
                    setReceivedQuantities(prev => ({ ...prev, [detail.QM4DROWID]: issuedQty.toFixed(3) }));
                    setReceivedBags(prev => ({ ...prev, [detail.QM4DROWID]: issuedBags.toFixed(0) }));
                    onQuantityReceivedChange?.(detail.QM4DROWID, issuedQty);
                    onBagsReceivedChange?.(detail.QM4DROWID, issuedBags);
                  }}
                >
                  Same
                </Text>
            ...
            <View style={styles.quantitySection}>
              ...
              <View style={styles.bagsGridRow}>
                  ...
                  <View style={[styles.quantityCard, { backgroundColor: colors.success + '08' }]}>
                    ...
                    <TextInput
                      style={[styles.quantityInput, { color: colors.success, backgroundColor: colors.cardBackground, borderColor: colors.divider }]}
                      value={receivedBags[detail.QM4DROWID] ?? (detail['BAGS RECEIVED'] !== null && detail['BAGS RECEIVED'] !== undefined ? (detail['BAGS RECEIVED'] === 0 ? '' : detail['BAGS RECEIVED'].toFixed(0)) : '')}
                      onChangeText={(text) => {
                        const numericValue = text.replace(/[^0-9.]/g, '');
                        setReceivedBags(prev => ({ ...prev, [detail.QM4DROWID]: numericValue }));
                        const numValue = parseFloat(numericValue);
                        if (!isNaN(numValue)) {
                          onBagsReceivedChange?.(detail.QM4DROWID, numValue);
                        }
                      }}
              ...
              <View style={styles.bagsGrid}>
                <View style={styles.bagsGridRow}>
                  ...
                  <View style={[styles.quantityCard, { backgroundColor: colors.success + '08' }]}>
                    ...
                    <TextInput
                      style={[styles.quantityInput, { color: colors.success, backgroundColor: colors.cardBackground, borderColor: colors.divider }]}
                      value={receivedQuantities[detail.QM4DROWID] ?? (detail['QUANTITY RECEIVED'] !== null && detail['QUANTITY RECEIVED'] !== undefined ? (detail['QUANTITY RECEIVED'] === 0 ? '' : detail['QUANTITY RECEIVED'].toFixed(3)) : '')}
                      onChangeText={(text) => {
                        const numericValue = text.replace(/[^0-9.]/g, '');
                        setReceivedQuantities(prev => ({ ...prev, [detail.QM4DROWID]: numericValue }));
                        const numValue = parseFloat(numericValue);
                        if (!isNaN(numValue)) {
                          onQuantityReceivedChange?.(detail.QM4DROWID, numValue);
                        }
                      }}
                      keyboardType="decimal-pad"
                      placeholder="0.000"
                ...
                <View style={styles.costItem}>
                  ...
                  <Text style={[styles.costValue, { color: colors.text }]}>
                    {(() => {
                      ...
                      const qtyReceived = parseFloat(receivedQuantities[detail.QM4DROWID]) || detail['QUANTITY RECEIVED'] || 0;
