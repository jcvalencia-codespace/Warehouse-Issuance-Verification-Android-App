...
export const MaterialUtilizationBatchLists: React.FC<MaterialUtilizationBatchListsProps> = ({
    usageNo,
    onBack,
    onAddNew,
    onAddBaseDetail,
    ...
    const renderBatchItem = ({ item }: { item: any }) => (
        <TouchableOpacity
            style={[styles.batchItem, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}
            onPress={() => onBatchPress?.(Number(item.BATCHNO), true)}
            activeOpacity={0.7}
        >
            <MaterialCommunityIcons name="package-variant-closed" size={20} color={colors.primary} style={styles.batchItemIcon} />
            <Text style={[styles.batchItemText, { color: colors.text }]}>Batch No: {item.BATCHNO}</Text>
    ...
    const renderNotDosingItem = ({ item }: { item: any }) => (
        <TouchableOpacity
            ...
            activeOpacity={0.7}
        >
            <MaterialCommunityIcons name="package-variant" size={20} color={colors.secondary} style={styles.batchItemIcon} />
            <Text style={[styles.batchItemText, { color: colors.text }]}>Batch No: {item.BATCHNO}</Text>
            <MaterialCommunityIcons name="chevron-right" size={20} color={colors.textSecondary} />
        </TouchableOpacity>
    );
    ...
    const renderBaseDetailEmpty = () => {
        return (
            <View style={styles.emptyContainer}>
                ...
                >
                    <MaterialCommunityIcons name="plus" size={20} color="#fff" />
                    <Text style={styles.createButtonText}>Create Material Utilization Base Detail.</Text>
                </TouchableOpacity>
            </View>
        );
    };
    ...
    return (
        ...
        >
            <View style={styles.headerBar}>
                ...
                <Text style={[styles.headerTitle, { color: colors.text }]}>Batch Lists</Text>
            </View>

            {loading ? (
                <View style={styles.loadingContainer}>
                    <ActivityIndicator size="large" color={colors.primary} />
                    <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Loading batch lists…</Text>
                </View>
            ) : (
                <View style={styles.content}>
                    {totalNotDosingItem > 0 && (
                        <View style={[styles.section, { marginTop: 24 }]}>
                            <View style={[styles.sectionHeader, { borderBottomColor: colors.cardBorder }]}>
                                <MaterialCommunityIcons name="robot-confused" size={22} color={colors.secondary} />
                                <Text style={[styles.sectionTitle, { color: colors.text }]}>Not Dosing Machine Items</Text>
                            ...
                            {notDosingBatchNos.length > 0 ? (
                                <FlatList
                                    ...
                                    keyExtractor={(item, index) => `notdosing-${item.BATCHNO ?? index}`}
                    ...
                    {totalDosingItem > 0 && (
                        <View style={styles.section}>
                            <View style={[styles.sectionHeader, { borderBottomColor: colors.cardBorder }]}>
                                ...
                                <Text style={[styles.sectionTitle, { color: colors.text }]}>Auto Dosing Machine Items</Text>
                            ...
                            {autoDosingBatchNos.length > 0 ? (
                                <FlatList
                                    ...
                                    keyExtractor={(item, index) => `auto-${item.BATCHNO ?? index}`}
                    ...
                    {totalDosingItem === 0 && totalNotDosingItem === 0 && (
